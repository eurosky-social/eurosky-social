locals {
  namespace = "ozone-berlin"
  hostname         = "live2025demo-ozone.${var.cluster_domain}"
  ozone_public_url = "https://${local.hostname}"
  ozone_appview_url = "https://api.bsky.app"
  ozone_appview_did = "did:web:api.bsky.app"
  ozone_server_did  = "did:plc:m4jxet5jry3f5xjxxedu6mt3"
  ozone_admin_dids  = "did:plc:m4jxet5jry3f5xjxxedu6mt3"
  ...
  ozone_configmap_yaml = templatefile("${path.module}/ozone-configmap.yaml", {
    ...
    ozone_appview_url = local.ozone_appview_url
    ozone_appview_did = local.ozone_appview_did
    ozone_server_did  = local.ozone_server_did
    ozone_admin_dids  = local.ozone_admin_dids
    pds_hostname      = local.pds_hostname
  })

  ozone_secret_yaml = templatefile("${path.module}/ozone-secret.yaml", {
    namespace              = local.namespace
    db_password_urlencoded = urlencode(var.ozone_berlin_db_password)
    ozone_admin_password   = var.ozone_berlin_admin_password
    ozone_signing_key_hex  = var.ozone_berlin_signing_key_hex
...
resource "kubernetes_config_map" "ozone_log_alerts" {
  ...
  data = {
    "ozone-log-alerts.yaml" = templatefile("${path.module}/log-alerts.yaml", {
      namespace = kubernetes_namespace.ozone.metadata[0].name
    })
  }
}
